...
export default getFinalGradeWithTime = async (
  type,
  lastQuestionMinutesLeft,
  lastQuestionSecondsLeft,
  setLastQuestionMinutesLeft,
  setLastQuestionSecondsLeft,
  ...
  grade = grade.toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1.");
  setTotalPointsWithTime(grade);
